...
async function renderMap(
  ...
  // Spring marker SVG centered at map center
  const mc = MARKER_COLOR[styleName] ?? '#e85d04';
  const markerSvg = Buffer.from(
    `<svg width="44" height="44" xmlns="http://www.w3.org/2000/svg">
      <circle cx="22" cy="22" r="20" fill="${mc}" fill-opacity="0.18"/>
      <circle cx="22" cy="22" r="13" fill="white"/>
      <circle cx="22" cy="22" r="8"  fill="${mc}"/>
    </svg>`
  );
  composites.push({
    input: markerSvg,
    left: Math.round(mapWidth  / 2) - 22,
    top:  Math.round(mapHeight / 2) - 22,
  });
